...
# -*- coding: utf-8 -*-
"""
HVAC 预测程序 - 完整版（集成三个步骤）

原有功能 + 新增：
✅ 步骤1：模型构建和比较（RF vs XGBoost vs DNN+LSTM）
✅ 步骤2：PSO 超参数优化
✅ 步骤3：SHAP 特征重要性分析
"""
...
try:
    ...
    print("[警告] model_comparison.py 未找到，步骤1 将被跳过")
...
try:
    ...
    print("[警告] pso_optimization.py 未找到，步骤2 将被跳过")
...
try:
    ...
    print("[警告] shap_analysis.py 未找到，步骤3 将被跳过")
...
设置你想运行的步骤：
- 步骤1（模型对比）：推荐启用，快速且有用
- 步骤2（PSO优化）：可选，耗时较长（30-60分钟）
- 步骤3（SHAP分析）：推荐启用，快速且有用
"""
ENABLE_STEP_1_COMPARISON = True   # ✅ 模型对比
ENABLE_STEP_2_PSO = False         # ❌ PSO 优化（耗时，初期推荐关闭）
...
INITIAL_BATCH_SIZE = int(os.environ.get('BATCH_SIZE', 384))
...
# ==================== 原有函数保持不变 ====================
# （这里省略了原有的所有函数，保持原样）
# 包括：try_enable_gpu, _clean_numeric_series, create_sliding_windows 等
...
def load_and_enhance_data(file_path):
    ...
    if len(target_cols) == 0:
        raise RuntimeError(f"未识别到供/回水目标列")
...
def build_optimized_model(input_shape, out_dim, model_scale=1.0):
    ...
    x = TimeDistributed(Dropout(0.15))(x)
    x = TimeDistributed(Flatten())(x)

    lstm1 = _scale(192)
    x_seq = Bidirectional(LSTM(lstm1, return_sequences=True))(x)
    ...
    lstm2 = _scale(128)
    x_lstm = tf.keras.layers.Reshape((1, x.shape[-1]))(x)
...
def main():
    ...
    print("\n正在加载和预处理数据...")
    file_path = 'git2.csv' if os.path.exists('git2.csv') else 'tempv3.csv'
    ...
    # 【步骤1】模型构建和比较
    # ============================================================
    if ENABLE_STEP_1_COMPARISON and STEP1_AVAILABLE:
        print("\n" + "="*60)
        print("【步骤1】模型构建和比较")
        print("="*60)
    ...
    elif ENABLE_STEP_1_COMPARISON and not STEP1_AVAILABLE:
        print("\n⚠️  步骤1 被启用但模块未找到，请确保 model_comparison.py 在同一目录")

    # ============================================================
    # 【步骤2】PSO 超参数优化
    # ============================================================
    if ENABLE_STEP_2_PSO and STEP2_AVAILABLE and len(X_train_dl) > 100:
        print("\n" + "="*60)
        print("【步骤2】PSO 超参数优化")
        print("="*60)
    ...
    elif ENABLE_STEP_2_PSO and not STEP2_AVAILABLE:
        print("\n⚠️  步骤2 被启用但模块未找到，请确保 pso_optimization.py 在同一目录")

    # ============================================================
    # 【步骤3】SHAP 特征重要性分析
    # ============================================================
    if ENABLE_STEP_3_SHAP and STEP3_AVAILABLE:
        print("\n" + "="*60)
    ...
    elif ENABLE_STEP_3_SHAP and not STEP3_AVAILABLE:
        print("\n⚠️  步骤3 被启用但模块未找到，请确保 shap_analysis.py 在同一目录")
